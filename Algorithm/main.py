...
which means 64 bits of plain text go as the input to DES, which produces 64 bits of ciphertext.
The same algorithm and key are used for encryption and decryption, with minor differences.
The key length is 56 bits.

# author: @balanton

# Python 3 implementation for DES algorithm
# Brief: https://en.wikipedia.org/wiki/Data_Encryption_Standard
"""
...
if __name__ == "__main__":
    input_str = "123456ABCD132536ADC111111111111111111111FACED1111111111111111111111111111111111111"
    input_chunks, _ = get_blocks(input_str, 16)
    key_str = "AABB09182736CCDD"

    round_keys_b, round_key_hex = get_round_keys(key_str)

    print("Plain Text: %s\n" % input_str)

    print("Encryption...")
    encrypted_message = encrypt_message(input_chunks, round_keys_b, round_key_hex)
    print("Cipher Text: %s\n" % encrypted_message)

    print("Decryption...")
    cipher_chunks, _ = get_blocks(encrypted_message, 16)
    decrypted_message = decrypt_message(cipher_chunks, round_keys_b, round_key_hex)
    print("Plain Text: %s\n" % decrypted_message)
